                      ...
                      <tr className="hover:bg-gray-50/50 cursor-pointer transition-colors" onClick={() => setExpandedId(expanded ? null : order.id)}>
                        ...
                        <td className="px-4 py-3 text-center relative" onClick={e => e.stopPropagation()}>
                          <button
                            ...
                            className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium hover:opacity-80 transition-opacity ${STATUS_STYLES[order.status]}`}
                          >
                            {order.status}
                          ...
                          {openMenuId === order.id && (
                            <>
                              ...
                              <div className="absolute right-4 top-full mt-1 w-44 bg-white border rounded-lg shadow-lg z-50 py-1 text-left">
                                <Link
                                ...
                                {canPay && (
                                  <button
                                    ...
                                    onClick={() => { updateStatus(order, 'Paid'); setOpenMenuId(null); }}
                                ...
                                {canComplete && (
                                  <button
                                    disabled={updatingId === order.id}
                                    onClick={() => { updateStatus(order, 'Completed'); setOpenMenuId(null); }}
                                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-blue-700 hover:bg-blue-50 disabled:opacity-50"
                                  >
                                    Mark Completed
                                  </button>
                                )}
                                {canCancel && (
                                  <button
                                    disabled={updatingId === order.id}
                                    onClick={() => { updateStatus(order, 'Cancelled'); setOpenMenuId(null); }}
