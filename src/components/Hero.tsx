import { useNavigate } from 'react-router-dom';
import { useHeroContent } from '../contexts/HeroContentContext';

export function Hero() {
  const navigate = useNavigate();
  const { content } = useHeroContent();

  const handleButtonClick = () => {
    if (/^https?:\/\//.test(content.buttonLink)) {
      window.location.href = content.buttonLink;
    } else {
      navigate(content.buttonLink);
    }
  };

  return (
    <section className="bg-white py-16 md:py-24 border-b border-gray-200">
      <div className="container mx-auto px-4">
        <div className="max-w-3xl">
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tight mb-4 text-[#3E2723]">
            {content.headline} <br />
            <span className="text-[#D32F2F]">{content.highlightedText}</span>
          </h1>
          <p className="text-xl font-medium mb-8 text-[#6D4C41]">
            {content.subtext}
          </p>
          <div className="flex flex-wrap gap-4">
            <button
              onClick={handleButtonClick}
              className="bg-[#D32F2F] text-white px-8 py-3 rounded-lg hover:bg-[#B71C1C] transition-colors"
            >
              {content.buttonText}
            </button>
          </div>
        </div>
      </div>
    </section>
  );
}
